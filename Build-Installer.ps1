...
function Write-InnoFilesInclude {
    ...
    foreach ($file in Get-ChildItem -Path $StageDir -Recurse -File | Sort-Object FullName) {
        ...
        if ($parent) { $dest = "{app}\$parent" }
        $source = "stage\$rel"
        $lines.Add("Source: ""$source""; DestDir: ""$dest""; Flags: ignoreversion")
    }
    if ($lines.Count -le 2) {
...
Write-Host "Wrote $generatedIss ($fileCount entries)"
...
try {
    ...
    do {
        ...
        $psi.Arguments = "/DMyAppVersion=$appVersion /Qp /Log=`"$logPath`" `"$issName`""
