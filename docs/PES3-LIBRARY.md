...
|------|-------------------|-------------|
| **Smart hybrid** (default) | Small **disc-assisted overlay** on SSD; bulk files read from the disc via links | Decrypt **once** into `library/titles/`; instant **Play from library** on later inserts |
...
Retail **encrypted** discs always need a decrypt step the first time you play. In **Smart hybrid**, that decrypt is promoted into the PES3 library automatically so repeat play is as fast as a manual dump — without copying ROMs off the disc by hand or downloading files online.
...
RPCS3/PES3/
  ...
  temp/
    ...
    session-*        ← ephemeral DIY sessions (retail uses library after first decrypt)
  state/
